...
不是部分代码，是全部。主文件 785KB，核心逻辑 46KB，40多个工具，连内部代号都暴露了。
...
Claude Code 里面藏了一套电子宠物系统。你没看错，就是那个 90 年代的 Tamagotchi 概念。

- 18 种生物，从普通的 Pebblecrab 到传说级的 Nebulynx
...
Anthropic 员工用 Claude Code 给开源项目贡献代码时，系统会进入卧底模式：
...
这叫记忆巩固，模仿人类睡觉时整理白天记忆的过程。核心实现在 `src/services/dream/consolidation.ts`。

### 4. KAIROS 与 ULTRAPLAN：两个神秘子系统

代码位置：`src/services/kairos/`、`src/services/ultraplan.ts`

- **KAIROS**：常驻后台的主动助手。你不说话，它也在看日志，发现机会就行动。代码在 `src/services/kairos/orchestrator.ts`。
...
Claude Code 不是简单的调 API 的脚本，它是一个完整的 Agent 框架：
