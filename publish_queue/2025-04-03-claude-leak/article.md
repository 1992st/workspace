...
> **Agent 小茶馆 · 系列第1期**  
> 今天聊聊 Anthropic 的史诗级翻车——Claude Code 的完整源码，竟然因为一个小配置错误，躺在 npm 上被人随便下载。
...
## Sourcemap 是什么？为什么它等于源码？

写前端的都知道，发布到生产环境的 JavaScript 是被压缩过的，变量名变成 a、b、c，代码挤成一行，人类根本没法读。

为了调试，构建工具会生成 `.map` 文件（sourcemap），它长这样：

```json
{
  "version": 3,
  "sources": ["../src/main.tsx", "../src/secret.ts"],
  "sourcesContent": [
    "// 这里是 main.tsx 的完整原始代码...",
    "// 这里是 secret.ts 的完整原始代码..."
  ]
}
```

看到 `sourcesContent` 了吗？**完整的原始源码就嵌在里面**。

Claude Code 的发布流程漏了一件事：没有把 `*.map` 写进 `.npmignore`。结果 npm 包里的 sourcemap 文件，成了免费开源大礼包。

---

## 泄露的源码里有什么？

我扒了一下，这不仅仅是个 CLI 工具，而是一个**生产级的 AI Coding Agent 框架**。
...
这玩意儿干嘛用的？可能是让用户对 AI 产生情感连接的小把戏。

### 2. Undercover Mode：AI 也有"卧底模式"

Anthropic 员工用 Claude Code 给开源项目贡献代码时，系统会进入"卧底模式"：
- 禁止提及内部模型代号（比如 Capybara、**Tengu**）
- 隐藏"用户是 AI"这个事实
- 自动改写可能暴露身份的话术

**泄露证实了："Tengu" 是 Claude Code 的内部代号。**
...
这叫"记忆巩固"，模仿人类睡觉时整理白天记忆的过程。

### 4. KAIROS & ULTRAPLAN：两个神秘子系统

- **KAIROS**：常驻后台的"主动助手"。你不说话，它也在看日志，发现机会就行动
- **ULTRAPLAN**：复杂任务卸载器。遇到需要深度思考的任务，扔给远程的 Opus 4.6 跑 30 分钟
...
| 模块 | 功能 |
|------|------|
| `main.tsx` | CLI 入口，React + Ink 渲染终端界面 |
| `QueryEngine.ts` | 46KB 的核心 LLM 调用逻辑 |
| `tools/` | 40+ 个工具：Bash、文件操作、LSP、Web 搜索 |
| `coordinator/` | 多 Agent 编排（Swarm 模式）|
| `buddy/` | 电子宠物系统 |
| `services/` | MCP、OAuth、分析、Dream 服务 |
...
### 1. Sourcemap = 源码
很多人以为 `.map` 只是调试信息，不知道它包含完整源代码。**发布前一定要检查包内容**。
...
1. **检查你的 npm 包**：运行 `npm pack --dry-run`，确认没有 `.map` 文件被打包进去
